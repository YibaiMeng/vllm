...
from vllm.v1.attention.ops.gdn_metadata import (
    prepare_full_graph_align_pure_spec_metadata,
)
...
class GDNAttentionMetadataBuilder(AttentionMetadataBuilder[GDNAttentionMetadata]):
    ...
    def _try_build_full_graph_align_pure_spec_metadata(
        self,
        ...
        if (
            m.block_table_tensor.device.type != "cuda"
        ...
        if not torch.equal(spec_sequence_masks_cpu, query_lens_cpu > 0):
            return None

        num_spec_decodes = int(spec_sequence_masks_cpu.sum().item())
        if not bool(spec_sequence_masks_cpu[:num_spec_decodes].all()) or bool(
            spec_sequence_masks_cpu[num_spec_decodes:].any()
        ):
            return None
        if (
            num_spec_decodes == 0
            or int(num_decode_draft_tokens_cpu[spec_sequence_masks_cpu].sum().item())
            == 0
        ):
            return None
        ...
        if (
            ...
            or num_spec_decode_tokens > self.decode_cudagraph_max_bs
            or num_spec_decode_tokens > num_spec_decodes * (self.num_spec + 1)
            or batch_size > self.decode_cudagraph_max_bs
            or self.num_spec > self.kv_cache_spec.num_speculative_blocks
            or self.num_spec + 1 > 16
            or bool((query_lens_cpu[:num_spec_decodes] > self.num_spec + 1).any())
        ):
            return None

        spec_token_size = min(
            num_spec_decodes * (self.num_spec + 1),
            int(m.query_start_loc_cpu[num_spec_decodes].item()),
        )
        prepare_full_graph_align_pure_spec_metadata(
    ...
    def build(  # type: ignore[override]
        ...
        fast_metadata = self._try_build_full_graph_align_pure_spec_metadata(
